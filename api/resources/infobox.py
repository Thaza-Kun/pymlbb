...
async def fetch_info_for_hero_with_users(
    ...
    cursor = await driver.conn.execute(
        ...
        "LEFT JOIN (SELECT hero_id, 100. * sum(is_team_win) / count(distinct match_id) as winrate FROM heroes_in_matches GROUP BY 1) total"
    ...
    for (
        ...
        if username in users:
            usersinfo[username] = UserInfo(
                username=username, winrate=user_winrate, hero_winrate=user_hero_winrate
            )
